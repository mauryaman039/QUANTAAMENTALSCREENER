...
def show_disclaimers():
    ...
    st.markdown("""
    * **RESEARCH ONLY:** Automated quantitative research tool. **NOT** financial advice.
    * **NO RECOMMENDATION:** Ratings are mathematical outputs based on public data.
    * **DATA ACCURACY:** We use public APIs. Data may be delayed or missing for some tickers.
    * **CONSULT PROFESSIONALS:** Investing involves a high risk of capital loss.
    """)
...
def fetch_comprehensive_data(ticker):
    try:
        ...
        profit_keys = ['Net Income', 'Net Income Common Stockholders', 'Net Profit', 'Profit After Tax']
        ...
        # 2. QUALITY DATA
        equity_keys = ['Stockholders Equity', 'Total Equity', 'Common Stock Equity']
        ...
        if m_cap_inr < 100_000_000_000: # Small
            cap_factor, cap_type = 0.75, "Small Cap"
        elif m_cap_inr < 500_000_000_000: # Mid
            cap_factor, cap_type = 0.65, "Mid Cap"
        elif m_cap_inr < 2_000_000_000_000: # Large
            cap_factor, cap_type = 0.55, "Large Cap"
        else: # Mega
            cap_factor, cap_type = 0.45, "Mega Cap"
            
        safe_growth = max(0.0, eps_cagr * cap_factor)
...
def display_pro_card(data):
    ...
    with st.container(border=True):
        ...
        with st.expander("Analysis Deep-Dive"):
            ...
            with t2: st.markdown(f"**Applied Growth:**\n{m['safe_growth']*100:.1f}%", help=f"{m['cap_type']} factor: {m['cap_factor']}")
...
with tab_screener:
    ...
    if st.button("🚀 Execute Global Quant Scan", type="primary"):
        ...
        for i, t in enumerate(tickers):
            ...
            # Slight delay to help prevent rate limiting during heavy scans
